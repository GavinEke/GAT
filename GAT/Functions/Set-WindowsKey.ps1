Function Set-WindowsKey {
    ...
    Process {
        $SoftwareLicensingService = Get-WmiObject -ComputerName $ComputerName -Class SoftwareLicensingService

        If ($PSCmdlet.ParameterSetName -eq "KMS") {
            $OSversion = Get-WmiObject -ComputerName $ComputerName -Class Win32_OperatingSystem | Select-Object -ExpandProperty Caption
